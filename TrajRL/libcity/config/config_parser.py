...
class ConfigParser(object):
    ...
    def _load_default_config(self):
        ...
        if self.config['model'] == 'MultiLayerGAT2PatchTSTContrastive':
            if 'split' in self.config and self.config['split'] is True:
                self.config['dataset_class'] = 'ContrastiveSplitDataset'
                self.config['executor'] = 'PatchContrastiveSplitExecutor'
        if self.config['model'] == 'MultiLayerGAT2PatchTSTContrastiveLM':
            if 'split' in self.config and self.config['split'] is True:
                self.config['dataset_class'] = 'ContrastiveSplitLMDataset'
                self.config['executor'] = 'PatchContrastiveSplitMLMExecutor'
        if self.config['model'] == 'GAT2PatchTSTContrastive':
            if 'split' in self.config and self.config['split'] is True:
                self.config['dataset_class'] = 'ContrastiveSplitDataset'
                self.config['executor'] = 'PatchContrastiveSplitExecutor'
        ...
        if self.config['model'] == 'PatchLinearClassify':
            if 'classify_label' in self.config and self.config['classify_label'] == 'usrid':
                self.config['evaluator'] = 'MultiClassificationEvaluator'
        default_file_list = []
        # model
