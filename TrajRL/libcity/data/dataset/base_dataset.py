...
class BaseDataset(AbstractDataset):
    def __init__(self, config):
        ...
        # patch
        self.patch_len = self.config.get('patch_len', self.seq_len)
        self.stride = self.config.get('stride', 1)
        self.mask_ratio = self.config.get('masking_ratio', 0.4)
        self.num_patch = self.config.get('num_patch', 1)
        self.win_size = self.config.get('win_size', 2)

        if self.add_gat or self.add_hgnn:
    ...
    def _load_geo_feature(self, road_info):
        ...
        if os.path.exists(node_fea_path):
            ...
            for k, v in norm_dict.items():
                ...
                node_features = node_features.drop(columns=k)  # drop报错
    ...
    def _gen_dataloader(self, train_dataset, eval_dataset, test_dataset):
        ...
        test_dataloader = DataLoader(test_dataset, batch_size=self.batch_size,
                                     ...
                                     collate_fn=lambda x: self.collate_fn(x, max_len=self.seq_len,
                                                                          vocab=self.vocab,
                                                                          add_cls=self.add_cls))  # 指定collate_fn函数，x为函数的默认参数，该默认参数的实值为Dataset.__getitem__()的返回值所组成的列表，长度为batch_size
    ...
    def get_data(self):
        ...
        return self._gen_dataloader(train_dataset, eval_dataset, test_dataset)  # 参数为三个dataset类
    ...
    def get_data_feature(self):
        data_feature = {'usr_num': self.usr_num, 'vocab_size': self.vocab_size, 'vocab': self.vocab,
                        ...
                        "loc_trans_prob": self.loc_trans_prob, "patch_len": self.patch_len,
                        "stride": self.stride, "mask_ratio": self.mask_ratio, "num_patch": self.num_patch,
                        "win_size": self.win_size}
        # 建立超图时会用到所有处理好的数据集(cache_dataset...),因此必须在getdata过程之后建立
        if self.add_hgnn:
            # build hypergraph
            print('Building hypergraph')
            self.node_features, self.node_fea_dim = self._load_geo_feature(self.geo_file)
            cachedatadir = './raw_data/' + self.dataset + '/cache_' + self.dataset
            train_path = cachedatadir + '_train_False_True_1.pkl'
            eval_path = cachedatadir + '_eval_False_True_1.pkl'
            test_path = cachedatadir + '_test_False_True_1.pkl'
            train_traj_list = pickle.load(open(train_path, 'rb'))
            eval_traj_list = pickle.load(open(eval_path, 'rb'))
            test_traj_list = pickle.load(open(test_path, 'rb'))
            all_trajs = ([traj[:, 0] for traj in train_traj_list] + [traj[:, 0] for traj in eval_traj_list]
                         + [traj[:, 0] for traj in test_traj_list])
            self.hypergraph = dhg.Hypergraph(self.vocab_size, all_trajs,merge_op='mean')  # road_num + 4 == vacab_size
            self.hypergraph = self.hypergraph.to(self.device)
        data_feature['hyper_graph'] = self.hypergraph
        return data_feature
...
class TrajectoryProcessingDataset(Dataset):
    ...
    def data_processing(self, origin_data, desc=None, cache_path=None, tmat_path=None):
        ...
        sub_data = origin_data[['path', 'tlist', 'usr_id', 'traj_id', 'vflag']]  # road_segment ID, timestamp list, …………
        traj_list = []
        temporal_mat_list = []
        for i in tqdm(range(sub_data.shape[0]), desc=desc):
            ...
            temporal_mat = self._cal_mat(tim_list)  # (real_seq_len, real_seq_len)
    ...
    def __getitem__(self, ind):
        ...
        return torch.LongTensor(traj_ind), torch.LongTensor(temporal_mat)  # 轨迹特征，时间矩阵
