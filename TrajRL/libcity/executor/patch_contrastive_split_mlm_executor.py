...
from libcity.utils.utils import cul_time_diff_list


class PatchContrastiveSplitMLMExecutor(PatchContrastiveMLMExecutor):
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Train epoch={}".format(
            ...
            time_diff_list1 = cul_time_diff_list(batch_temporal_mat1, padding_masks1, self.time_interval_scale_list,
                                                 self.max_interval)
            time_diff_list2 = cul_time_diff_list(batch_temporal_mat2, padding_masks2, self.time_interval_scale_list,
                                                 self.max_interval)
            time_diff_list = cul_time_diff_list(batch_temporal_mat, padding_masks, self.time_interval_scale_list,
                                                 self.max_interval)
            graph_dict = self.graph_dict

            z1, z2, predictions_l,mask = self.model(contra_view1=contra_view1, contra_view2=contra_view2,
                                               argument_methods1=self.data_argument1,
                                               argument_methods2=self.data_argument2,
                                               masked_input=X, padding_masks=padding_masks,
                                               batch_temporal_mat=time_diff_list,
                                               padding_masks1=padding_masks1,
                                               batch_temporal_mat1=time_diff_list1,
                                               padding_masks2=padding_masks2,
                                               batch_temporal_mat2=time_diff_list2,
                                               graph_dict=graph_dict)
            # (B, d_model), (B, d_model), (B, T, vocab_size), (B, T, 1441)
            targets_l = targets[...,0]
            masks_l = torch.flatten(mask)
            # targets_l, target_masks_l = targets[..., 0], target_masks[..., 0]
            mean_loss_l, batch_loss_l, num_active_l = self._cal_loss(predictions_l, targets_l, masks_l)
            ...
            with torch.no_grad():
                total_correct_l += self._cal_acc(predictions_l, targets_l, masks_l)
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="{} epoch={}".format(
                ...
                if mode == 'Test':
                    evaluate_input = {
                        'loc_true': targets_l[masks_l].reshape(-1, 1).squeeze(-1).cpu().numpy(),  # (num_active, )
                        'loc_pred': predictions_l[masks_l].reshape(-1, predictions_l.shape[-1]).cpu().numpy()
                        # (num_active, n_class)
                    }
