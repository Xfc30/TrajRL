...
from libcity.utils.utils import cul_time_diff_one, cul_time_diff_list


class PatchContrastiveExecutor(PatchTSTBaseExecutor):
    ...
    def _contrastive_loss_consert(self, z1, z2):
        ...
        if self.similarity == 'inner':
            ...
            logits_aa = F.cosine_similarity(z1.unsqueeze(1), hidden1_large.unsqueeze(0),
                                            dim=-1) / self.temperature  # shape (bsz, bsz)
            logits_bb = F.cosine_similarity(z2.unsqueeze(1), hidden2_large.unsqueeze(0),
                                            dim=-1) / self.temperature  # shape (bsz, bsz)
            logits_ab = F.cosine_similarity(z1.unsqueeze(1), hidden2_large.unsqueeze(0),
                                            dim=-1) / self.temperature  # shape (bsz, bsz)
            logits_ba = F.cosine_similarity(z2.unsqueeze(1), hidden1_large.unsqueeze(0),
                                            dim=-1) / self.temperature  # shape (bsz, bsz)
        else:
            logits_aa = torch.matmul(z1, hidden1_large.transpose(0, 1)) / self.temperature  # shape (bsz, bsz)
            logits_bb = torch.matmul(z2, hidden2_large.transpose(0, 1)) / self.temperature  # shape (bsz, bsz)
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader=None):
        ...
        if self.load_best_epoch:
            self._logger.info('best epoch is {}. Loading...'.format(best_epoch))
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Train epoch={}".format(
            ...
            time_diff_list = cul_time_diff_list(batch_temporal_mat, padding_masks, self.time_interval_scale_list,
                                                self.max_interval)
            graph_dict = self.graph_dict

            z1 = self.model(x=X1, batch_temporal_mat=time_diff_list, argument_methods=self.data_argument1,
                            graph_dict=graph_dict)  # (batch_size, d_model)
            z2 = self.model(x=X2, batch_temporal_mat=time_diff_list, argument_methods=self.data_argument2,
                            graph_dict=graph_dict)  # (batch_size, d_model)
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="{} epoch={}".format(
                ...
                padding_masks = padding_masks.to(self.device)  # 0s: ignore
                batch_temporal_mat = batch_temporal_mat.to(self.device)
                time_diff_list = cul_time_diff_list(batch_temporal_mat, padding_masks, self.time_interval_scale_list,self.max_interval)
                z1 = self.model(x=X1, batch_temporal_mat=time_diff_list, argument_methods=self.data_argument1,
