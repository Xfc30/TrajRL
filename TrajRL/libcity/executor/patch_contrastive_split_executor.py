...
from libcity.executor import PatchContrastiveExecutor
...
class PatchContrastiveSplitExecutor(PatchContrastiveExecutor):
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Train epoch={}".format(
            ...
            z1 = self.model(x=X1, batch_temporal_mat=time_diff_list1,
                            argument_methods=self.data_argument1, graph_dict=graph_dict)  # (batch_size, d_model)
            z2 = self.model(x=X2, batch_temporal_mat=time_diff_list2,
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="{} epoch={}".format(
                ...
                time_diff_list2 = cul_time_diff_list(batch_temporal_mat2, padding_masks2, self.time_interval_scale_list,
                                                     self.max_interval)
                z1 = self.model(x=X1, batch_temporal_mat=time_diff_list1, argument_methods=self.data_argument1,
                                graph_dict=self.graph_dict)  # (batch_size, d_model)
                z2 = self.model(x=X2, batch_temporal_mat=time_diff_list2, argument_methods=self.data_argument2,
                                graph_dict=self.graph_dict)  # (batch_size, d_model)
